...
app.post("/products", (req,res)=>{
  ...
  res.json({ message: "Da them san pham " });
  
    
})
app.delete("/products/:id", (req,res)=>{
  const index = products.indexOf(
    products.find((value) => value.id == req.params.id)
  );
  products.splice(index, 1);
  res.json({message: "data deleted successfully!",data: products});
})
app.put("/products/:id", (req,res)=>{
  ...
  products.splice(index, 1);
  res.json({message: "update successfully!",data: products});
})
